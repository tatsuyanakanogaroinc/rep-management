-- SNS Management System Database Migration
...
-- 2. Create growth_parameters table if it doesn't exist
CREATE TABLE IF NOT EXISTS public.growth_parameters (
...
-- Insert default growth parameters if table is empty
...
SELECT 30, 50.00, 2490, 24900, 5.00, true
WHERE NOT EXISTS (SELECT 1 FROM public.growth_parameters WHERE is_active = true);

-- 3. Create targets table if it doesn't exist
...
-- Insert default target data from spreadsheet if table is empty
DO $$
DECLARE
    target_exists BOOLEAN;
BEGIN
    SELECT EXISTS(SELECT 1 FROM public.targets LIMIT 1) INTO target_exists;
    
    IF NOT target_exists THEN
        -- Insert target data for 2025-09 through 2026-08
...
-- 6. Create RLS policies for growth_parameters table
...
-- 7. Create RLS policies for targets table
...
-- 8. Fix tatsuya.nakano@garoinc.jp user to be admin
